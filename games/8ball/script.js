...
function resize() {
  ...
  // clientWidth/Height include padding, so take it back off before dividing
  // the room between the table and the control column.
  const cs = getComputedStyle(stage);
  const padX = parseFloat(cs.paddingLeft) + parseFloat(cs.paddingRight);
  const padY = parseFloat(cs.paddingTop) + parseFloat(cs.paddingBottom);
  const gap = parseFloat(cs.columnGap) || 0;
  const sideW = $('#side').offsetWidth || 52;

  const availW = stage.clientWidth - padX - sideW - gap;
...
/* Every ball, solids down the left and stripes down the right, the eight
   below. A ball lights up once it is off the table. */
const ballCells = [];

function buildBallColumn() {
  const wrap = $('#balls');
  if (!wrap || wrap.childElementCount) return;

  const cell = (id, wide) => {
    const d = document.createElement('span');
    d.className = 'bp ' + (id > 8 ? 'stripe' : 'solid') + (wide ? ' eight' : '');
    d.style.setProperty('--bc', SPEC[id]);
    d.innerHTML = '<b>' + id + '</b>';
    ballCells.push({ id: id, el: d });
    return d;
  };

  for (let n = 1; n <= 7; n++) { wrap.appendChild(cell(n)); wrap.appendChild(cell(n + 8)); }
  wrap.appendChild(cell(8, true));
}

function paintBalls() {
  buildBallColumn();
  for (const c of ballCells) {
    const b = G.balls.find(x => x.id === c.id);
    c.el.classList.toggle('down', !!(b && b.out));
  }
}
...
function syncUI() {
  const base = G.mode === 'ai' ? 'Vs. computer · ' + G.diff : 'Two players';
  const mine = G.groups[G.turn];
  $('#matchLabel').textContent = mine
    ? base + ' · ' + playerName(G.turn) + ': ' + GROUP_NAME[mine].toLowerCase()
    : G.open ? base + ' · table open' : base;
  ...
  ['#strike', '#aimLeft', '#aimRight', '#resetShot']
