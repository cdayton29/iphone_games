...
(function () {
  ...
  function sizeGrid() {
    ...
    // clientWidth/clientHeight include padding, so subtract it — otherwise the
    // grid is sized to the padding box and spills past both edges.
    const cs = getComputedStyle(stage);
    const padX = parseFloat(cs.paddingLeft) + parseFloat(cs.paddingRight);
    const padY = parseFloat(cs.paddingTop) + parseFloat(cs.paddingBottom);

    // An iframe can report zero size on its first frame; fall back sensibly.
    const w = (stage.clientWidth || document.documentElement.clientWidth || 360) - padX;
    const h = (stage.clientHeight || (document.documentElement.clientHeight - 200) || 360) - padY;

    // Floor to a multiple of 9 so all nine columns are exactly equal.
    const size = Math.floor(Math.max(0, Math.min(w, h)) / 9) * 9;
  ...
  function newPuzzle(clues) {
    ...
    sizeGrid();
    render();
    requestAnimationFrame(sizeGrid);   // settle once the iframe has real dimensions
  }
